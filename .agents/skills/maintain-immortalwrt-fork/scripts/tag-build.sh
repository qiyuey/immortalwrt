...
require_clean_tree
"$SCRIPT_DIR/verify-single-patch.sh" >/dev/null || \
    fail "single-FIX invariant failed; run verify-single-patch.sh for details"

BASE_TAG=$(git describe --tags --abbrev=0 --exclude='my/*' --exclude='backup/*' 2>/dev/null || true)
